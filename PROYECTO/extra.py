...
class JuegoClasicoExtra:
    ...
    barcos_requeridos = 5
    # Cuando True, el jugador conserva el turno si su disparo fue un impacto
    ...
    def disparar(self, jugador_actual, id_objetivo, fila, columna):
        ...
        # Si `extra_turn_on_hit` está activado, el jugador conserva el turno cuando
        # el resultado es "impacto" (incluye hundir barcos).
        if self.estado != "terminada":
            if not (self.extra_turn_on_hit and resultado == "impacto"):
                self.cambiar_turno()
